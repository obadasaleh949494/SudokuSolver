...
def BuildImage (string,arr ):
    if len(string) <81 :
        ...
        return
    img = PIL.Image.new('RGB', (550, 550), color = (255, 255, 255))
...
class hamzaButton:
    def __init__(self,master):
        ...
        self.Editmenu.add_command(label="Description" )
        self.button=Button(master,text="Brows" ,height = 2, width = 10,bg="#b4bcca",command=self.brows)
        self.button2=Button(master,text="Solve",height = 2, width = 10,bg="#b4bcca",command=self.printMSG)
        self.button2.config(font=('bold', 12))
        self.button.config(font=('bold', 12))
        self.frame=Frame(master,width=500,height=300)
        self.frame.config(background="#696b78")
        self.frame.grid(columnspan=8)
        self.button.grid(row=1,column=0,pady=3)
        self.button2.grid(row=2,column=0,pady=3)
        self.label=Label(self.frame)
        self.label1=Label(self.frame)

    def printMSG(self):
        if not self.path=='':
            DeleteLines(self.path)
            DeleteLines('FirstProcessing.png')
            improveImage()
            DividImage()
            st=ImageData()
            arr=EmptyCell(st)
            ...
            self.label1=Label(self.frame,image=photo)
            self.label1.image=photo

            self.label1.pack(side=RIGHT)
        else :
            tkinter.messagebox.showinfo("Warning","you have to insert a sudoku image to solve")


    def brows(self):
            r =  filedialog.askopenfilename(initialdir = "/",title = "Select file",filetypes = (("jpeg files","*.jpg"),("all files","*.*")))
            self.path=r
            if not r=='':
                from PIL import Image, ImageTk
